  ...
  find(input) {
    ...
    */
    ...
    const mainExtractionAlgorithm = (
      wordsStr,       // the chunk (ie. the actual piece of text)
      positions,      // global positions of sentences, words, characters
      globalResults   // buffer to collect the results
    ) => {
      ...
      for (let n = 1; n <= maxLength; n++) {
        ...
        for (let wordIndex = 0; wordIndex <= (wordsArr.length - n); wordIndex++) {

          // get a fresh slice of n-gram from the array
          const ngramArr = wordsArr.slice(wordIndex, wordIndex + n)
          ...
          const headLength = (typeof wordsArr[wordIndex] !== 'undefined') ? wordsArr[wordIndex].length : 0
    ...
    for (const character of input) {
      ...
      if(isEnding && !(isBreakingChar || isSpacingChar)) {
        ...
        mainExtractionAlgorithm(buffer.join(''), positions, globalResults)
      ...
      if (buffer.length > 6 && isBreakingChar) {
        isEnding = true
      }
